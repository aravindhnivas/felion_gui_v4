...
<script>
    ...
    $: eV = Number((PlanksConstant / electricCharge) * parseFloat(hz)).toFixed(fixedDigit)
    $: kelvin = Number((PlanksConstant / boltzmanConstant) * parseFloat(hz)).toFixed(
        fixedDigit
    )

    $: cm_1 = Number(parseFloat(hz) / (SpeedOfLight * 1e2)).toFixed(fixedDigit)
    $: um = Number((SpeedOfLight / parseFloat(hz)) * 1e6).toFixed(fixedDigit)
    $: ghz = Number(parseFloat(hz) * 1e-9).toFixed(fixedDigit)
    $: nm = Number((SpeedOfLight / parseFloat(hz)) * 1e9).toFixed(fixedDigit)
    $: J = Number(PlanksConstant * parseFloat(hz)).toExponential(fixedDigit)
