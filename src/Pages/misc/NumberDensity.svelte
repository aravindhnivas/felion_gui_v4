<script lang="ts">
    ...
    let F: string = ''
    let srgMode = false
    let added_pressure: string = '5e-6'
    let trap_temperature: string = '4.7'
    let background_pressure: string = '1e-8'
    let calibration_factor: number = $C_factor
...
<div class="align h-center">
    ...
    <Switch
        on:change={dispatch_current_numberdensity}
...
>
    <div style="display: flex; flex-direction: column; padding: 0 1em;">
        <div class="align h-center">
            <div class="border__div">
                <Textfield bind:value={trap_temperature} label={'trap_temperature [K]'} />
                <Textfield bind:value={$number_density_std.trap_temperature} label="std. dev." />
            </div>
            <div class="border__div">
                <Textfield bind:value={background_pressure} label={'background_pressure [mbar]'} />
                <Textfield bind:value={$number_density_std.background_pressure} label="std. dev. (%)" />
            </div>
            <div class="border__div">
                <Textfield bind:value={added_pressure} label={'added_pressure [mbar]'} />
                <Textfield bind:value={$number_density_std.added_pressure} label="std. dev. (%)" />
            </div>
        </div>
    </div>
    ...
    <div style="display: flex; flex-direction: column; padding: 0 1em;">
        <div class="align h-center">
            <div class="border__div">
                <Textfield disabled={srgMode} bind:value={calibration_factor} label="Calibration Factor" />

                <Textfield disabled={srgMode} bind:value={$number_density_std.calibration_factor} label="std.dev." />
            </div>

            <div class="border__div">
                <Textfield bind:value={$room_temperature} label="Room temperature (K)" />

                <Textfield bind:value={$number_density_std.room_temperature} label="std.dev." />
            </div>
        </div>
        <div class="align h-center constants">
            <div class="flex items-end">
                <h2 class="mt-5" style:width="100%">Modified Takaishi-Sensui constants</h2>
                <button
                    class="i-mdi-open-in-new"
                    on:click={async () => {
                        const [err] = await oO(shell.open('https://doi.org/10.1063/1.481976'))
                        if (err) window.createToast('Failed to open link', 'danger')
                    }}
                />
            </div>
            <br />
            ...
            <div class="border__div">
                ...
                />
                <Textfield bind:value={$number_density_std.tube_diameter} label="std. dev." />
            </div>

            <div class="border__div">
                <Textfield disabled value={X || ''} label="X [mm.Pa / K]" />

                <Textfield disabled value={F || ''} label="F (Degree of thermal transpiration)" />
            </div>
        </div>
    </div>
</div>
